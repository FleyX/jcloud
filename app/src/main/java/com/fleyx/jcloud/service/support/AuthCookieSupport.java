 ...
 * <p>
 * 访问令牌与刷新令牌均以 HttpOnly、SameSite=Strict、Path=/ 写入；刷新令牌为持久 cookie，
 * Max-Age 跟随 {@link AuthProperties#getRefreshExpireDays()} 的滑动有效期，访问令牌 Max-Age
 * 跟随 JWT 有效期（{@link JwtProperties#getExpireHours()}）；Secure 由配置项 {@code cookieSecure}
 * 控制、默认关闭以兼容局域网纯 HTTP。
 */
@Component

 ...
 * 默认缓存 5 分钟，角色变更后应调用 {@link #evict(Long)} 清除对应用户缓存。
 */
@Component
public class UserPermissionCache {
    ...
    private final Map<Long, CacheValue> cache = new ConcurrentHashMap<>();
     ...
     */
    public record CacheValue(List<Long> roleIds, List<String> resourceCodes, boolean superAdmin, long expireAt) {
    }
     ...
     * @param superAdmin    是否为超级管理员
     */
    public void put(Long userId, List<Long> roleIds, List<String> resourceCodes, boolean superAdmin) {
     ...
     */
    public CacheValue get(Long userId) {

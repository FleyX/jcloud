  ...
  <div class="flex h-screen w-screen items-center justify-center bg-gradient-to-br from-surface-50 to-primary-50 p-4">
    <div class="w-full max-w-md rounded-3xl bg-white p-8 shadow-soft">
      ...
      <div class="mb-8 flex flex-col items-center gap-3">
        ...
        <h1 class="text-2xl font-bold text-surface-900">欢迎回到 JCloud</h1>
        <p class="text-sm text-surface-500">请输入账号密码登录系统</p>
      </div>
      ...
      <form class="space-y-5" @submit.prevent="handleLogin">
        ...
        <div>
          ...
          <div class="relative">
            ...
            >
              <component :is="showPassword ? EyeOff : Eye" class="h-[18px] w-[18px]" />
            </button>
          </div>
        </div>
      ...
      <div class="mt-6 text-center text-sm text-surface-500">
        还没有账号？
        <button class="font-medium text-primary-600 hover:text-primary-700" @click="goRegister">立即注册</button>
      </div>
    </div>
  </div>
</template>

    ...
    <DialogPortal>
      ...
      <DialogContent class="fixed left-1/2 top-1/2 w-full max-w-xl -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-white p-6 shadow-xl">
        ...
        <DialogDescription class="mt-1 text-sm text-surface-500">
          {{ isEdit ? '修改权限信息与资源' : '创建新权限' }}
        </DialogDescription>

        <div
          v-if="isEdit"
          class="mt-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-700"
        >
          非必要请勿修改，避免系统出错
        </div>
        ...
        <div class="mt-4 space-y-4">
          ...
          <div>
            ...
            <input
              v-model="form.name"
              type="text"
              class="w-full rounded-xl border border-surface-200 px-3 py-2 text-sm"
            >
          </div>
          <div>
          ...
          <div>
            ...
            <div class="mb-2 flex gap-2">
              <select
                ...
                class="rounded-xl border border-surface-200 px-3 py-2 text-sm"
              >
                <option
            ...
            <div class="max-h-48 overflow-y-auto rounded-xl border border-surface-200 p-3">
              ...
              <p
                ...
                class="py-2 text-center text-sm text-surface-400"
              >
                无匹配资源
              </p>
            </div>

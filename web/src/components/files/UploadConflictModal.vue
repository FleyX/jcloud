...
/**
 * 上传冲突解决弹窗。
 * 支持批量冲突：每项选择跳过/覆盖/自动重命名，并提供全部应用按钮。
 */
...
function isFolderConflict(conflict: ConflictItemVo): boolean {
  return conflict.existingType === 'folder'
}
</script>
        ...
        <div class="mb-5 max-h-64 space-y-3 overflow-y-auto">
          ...
          >
            ...
            <p class="mb-3 text-xs text-surface-500">
              目标位置已存在同名
              <span class="font-medium text-surface-700">{{ isFolderConflict(conflict) ? '文件夹' : '文件' }}</span>
              <span class="mx-1 font-medium text-surface-900">"{{ conflict.existingName }}"</span>
            </p>
            <div class="grid grid-cols-3 gap-2">
              <button
                v-for="strategy in (['skip', 'overwrite', 'auto_rename'] as ConflictStrategy[])"
                :key="strategy"
                :class="cn(
